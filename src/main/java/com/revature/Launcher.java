...
public class Launcher {
	...
	public static void main(String[] args) {
		...
		Javalin app = Javalin.create(
				...
				).start(3004); //we need this to start our application on port 3000
		...
		app.post("/login", lc.loginAttempt);
        app.get("/logout", lc.logout);
        
        app.get("/requests/all", rc.getAllRequests);
        app.get("/requests/{reimbStatus}", rc.getByStatus);
        app.post("/requests/add", rc.addRequest);
        app.patch("/requests/resolve", rc.resolveRequest);
    }
